...
class DisplayTagTest extends DfpTestBase {
  ...
  public function testDisplayTag() {
    ...
    $this->assertSession()
      ->responseContains('googletag.defineSlot("' . $tag_view->getAdUnit() . '", ' . $tag_view->getSize() . ', "' . $tag_view->getPlaceholderId() . '")');
    $this->assertSession()->responseContains('gads.async = true');
    $this->assertSession()
    ...
    // Check that sync mode outputs the right JS.
    $this->config('dfp.settings')->set('async_rendering', FALSE)->save();
    $this->drupalGet('<front>');
    $this->assertSession()->responseNotContains('gads.async = true');
    $this->assertSession()
      ->responseContains(TagInterface::GOOGLE_TAG_SERVICES_URL);
    $this->assertSession()
      ->responseContains('googletag.pubads().enableSyncRendering()');
  }
  ...
  public function testDisplayTagWithMapping() {
    ...
    $this->assertSession()
      ->pageTextContains($this->t('The browser size can only contain numbers and the character x.'));
    ...
    $this->dfpEditTag($tag->id(), $edit);
    $this->assertSession()
      ->pageTextContains($this->t('The ad size(s) string can only contain numbers, the character x and commas (unless it is the special keyword "<none>").'));
  ...
  public function testTargeting() {
    ...
    $this->dfpEditTag($tag->id(), $edit);
    $this->assertSession()
      ->pageTextContains($this->t('The value cannot be empty if a target exists.'));
    ...
    $this->dfpEditTag($tag->id(), $edit);
    $this->assertSession()
      ->pageTextContains($this->t('The target cannot be empty if a value exists.'));
